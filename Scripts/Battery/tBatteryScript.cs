...
public class tBatteryScript : MonoBehaviour
{
    ...
    {
        // 初回に1度nullで処理に来てエラーが出ていたため、nullチェックをするように修正
        if (statusBattery_ != null)
        {
            return ((statusBattery_[(int)type.LEFT].batteryImage.fillAmount + statusBattery_[(int)type.RIGHT].batteryImage.fillAmount) / 2) * 100;
        }
        return 100.0f;
    }
}
